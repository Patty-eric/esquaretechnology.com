...
$cn=mysqli_connect("localhost","id17828412_esquaretech","Nibishaka@123");
                mysqli_select_db($cn,"id17828412_esquare");
...
$stmt = $user_home->runQuery("SELECT* FROM applied WHERE id=:uid");
$stmt->execute(array(":uid"=>$_SESSION['userSession']));
$row = $stmt->fetch(PDO::FETCH_ASSOC);
$a=$row['id'];

function check_dep($cn,$id){
	$sql=mysqli_query($cn,"SELECT id FROM `users` WHERE `id`='$id' ");
	$row=mysqli_fetch_array($sql);
	return $row['id'];
}
/*
function check_org($cn,$id){
	$sql=mysqli_query($cn,"SELECT `lname` FROM `organizations` WHERE `userID`='$id' ");
	$row=mysqli_fetch_array($sql);
	return $row['lname'];
}*/
?>
...
$cn=mysqli_connect("localhost","id17828412_esquaretech","Nibishaka@123");
                mysqli_select_db($cn,"id17828412_esquare");
                $sasa="SELECT* FROM users WHERE users.id={$_SESSION['userSession']}"; 
                $baa=mysqli_query($cn,$sasa);
                $rows=mysqli_fetch_array($baa);
                
echo "Welcome:".$rows['userEmail']; ?>
				...
				<ul class="nav nav-list">
					<li>
						...
						<a href="dashboard.php">
							...
							<span class="menu-text">Apply Other Post </span>
						</a>
								...
								<div class="row">
									<div class="col-xs-12">
										 <table id="example" class="table table-striped table-bordered">
											...
											<tbody>
												<?php
                //$cn = new PDO('mysql:host=localhost; dbname=id17828412_esquare','id17828412_esquaretech', 'Nibishaka@123');
                $cn=mysqli_connect("localhost","id17828412_esquaretech","Nibishaka@123");
                mysqli_select_db($cn,"id17828412_esquare");
                
                $q="SELECT* FROM applied,users WHERE applied.applicant_code=users.id && users.id={$_SESSION['userSession']}";
               // $result = $cn->prepare("SELECT* FROM applied,users WHERE applied.id='".$_SESSION['userSession']."'");
                $b=mysqli_query($cn,$q);
                for($i=0; $row =mysqli_fetch_array($b); $i++){
                //$id=$row['id'];
              ?>
												...
												<tr>
													...
													<td><p>Applicant Code:</p><?php echo check_dep($cn,$row ['applicant_code']); ?></td>
													<td>
														<p>Task Name:</p><?php echo $row ['Task_Name']; ?>
													</td>
													<td>
														<p>Task Type:</p><?php echo $row ['Task_Type']; ?>
													</td>
													
													<td>
														<p>Your Skills:</p><?php echo $row ['Required_Skills']; ?>
													</td>
													...
													<td>
													    <p>User Status:</p>
													<?php
													if($row['userStatus']=="N") { echo '<span class="label label-sm label-warning">Pending</span>'; }
													if($row['userStatus']=="Y") { echo '<span class="label label-sm label-success">Start Task Now</span>'; }
													if($row['userStatus']=="C") { echo '<span class="label label-sm label-danger">Rejected</span>'; }
														if($row==0){
												     echo"<button class='btn btn-warning'>No Task Assigned to You<button>";	 
												 }
													?>
														
													</td>
													
													<td><p>Start Date:</p><?php echo $row ['Start_Date']; ?></td>
													<td><p>End Date:</p><?php echo $row ['end_of_Project']; ?></td>
													<td><p>Reward:</p><?php echo $row ['Reward']; ?></td>
												</tr>
