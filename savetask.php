				...
				<ul class="nav nav-list">
					<li class="active">
						...
						<a href="dashboard.php">
							...
							<span class="menu-text">Apply Other Task </span>
						</a>
...
if(isset($_POST['btn-signup'])){
  ...
  $requiredskills=$_POST['required_task'];
  ...
  //$error=array();
  //$b=$_GET['id'];
  $select="SELECT Task_Name FROM applied WHERE Task_Name='$taskname'";
  $selected_data=mysqli_query($con,$select);
  if(mysqli_num_rows($selected_data)>0){
      echo'<p class="btn btn-warning">You are arleady applied to this post</p> <a href="dashboard.php" class="btn btn-info">Back to Apply On Other Post Please</a>';
      exit;
  }
  $_SESSION['userEmail']='userEmail';
 $all="insert into applied  values(NULL,'$applicant_code','$applicant_fname','$applicant_lname','$taskname','$tasktype','$requiredskills','$startdate','$end','$reward')";
 $my_all=mysqli_query($con,$all);
  if($my_all){
    echo'<center>';
     echo'<script>alert("Your application received soon you will be contacted Press ok continue")
   ...
   // header('location:dashboard');
        ...
        echo'<script>alert("data failed to be applied")</script><a href="dashboard.php" class="btn btn-info">Back</a>';
     }}
 //print_r($all);
